...
In the different Whonix types, the two files to be edited are in different places. If using Qubes-Whonix read how to get your IP.
- Create a [HiddenService on Whonix-Gateway](https://www.whonix.org/wiki/Onion_Services#Step_2:_Edit_Tor_Configuration)
- [Open Whonix-Workstation Firewall Port 9999](https://www.whonix.org/wiki/Onion_Services#Step_2:_Open_Whonix-Workstation_Firewall_Port)

## non-Qubes Whonix running in VirtualBox or KVM - Whonix with Xfce graphical user interface (GUI)

Configure a HiddenService on the Whonix-Gateway

!!! info
    There is a provided torrc.examples & Tor User Config in the GUI Application Menue<br>
...
Hint: `HiddenServiceVersion 3 is not required`, this is the Tor default. v2 hasn't been supported in Tor for years.

Reload Tor config to create the HiddenService with: `sudo systemctl reload tor`
And get Your_HiddenService_address `sudo cat /var/lib/tor/haveno_service/hostname`
...
!!! info
    There is Global Firewall Settings in the GUI Application Menue where it says:
...
??? info
    ...
    `sudo dpkg -i haveno-v*-linux-x86_64-installer.deb`<br>
Haveno Launcher should be in `Applications` -> `Internet` You can edit it to<br>
`/opt/haveno/lib/haveno-Haveno.desktop --hiddenServiceAddress=Your_HiddenService_address.onion --nodePort=9999`
!!! info
    Your_HiddenService_address is the saved output from Whonix-Gateway `sudo cat /var/lib/tor/haveno_service/hostname`

If not create a desktop shortcut: copy (or drag) `/opt/haveno/lib/haveno-Haveno.desktop --hiddenServiceAddress=Your_HiddenService_address.onion --nodePort=9999` to your desktop
